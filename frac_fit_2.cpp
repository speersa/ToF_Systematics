...
void convolute_1d(TH1* mc, TH1* kernel, TH1D*convoluted_mc){
    ...
    for (int i_conv = 0; i_conv < 2 * nmcbins - 1; i_conv ++){
		...
		full_convoluted.at(i_conv) += mc->GetBinContent(k + 1) * kernel->GetBinContent(i_conv - k + 1);
        }
    ...
    for (int i_result = 0; i_result < nmcbins; i_result++){
        if (nmcbins%2 == 1){
			...
			convoluted_mc->SetBinContent(i_result + 1, full_convoluted.at(i_result + nmcbins/2));
        }
        else {
            //convoluted_mc->SetBinContent(i_result + 1, full_convoluted[i_result + nmcbins/2 - 1]);
			convoluted_mc->SetBinContent(i_result + 1, full_convoluted.at(i_result + nmcbins/2 - 1));
        }
...
std::string output_folder = "/home/t2k/aspeers/ToF_Systematics/frac_fit_plots_2/";
...
TFile *file_mc = TFile::Open("/home/t2k/aspeers/ToF_Systematics/ToF_Output_P7_MC_run6_run7.root");
TFile *file_data = TFile::Open("/home/t2k/aspeers/ToF_Systematics/ToF_Output_P7E_RHC_Data.root");
...
for (int i = 0; i < 24; i++){
    ...
    if (myfile.is_open())
    {
        myfile << Form("%i %s Sigma F: %.4f Sigma B: %.4f MuF:%.4f MuB:%.4f FP:%.4f FM%.4f \n", i, sample_names[i].c_str(), bestfit_parameters[i][0], bestfit_parameters[i][5], bestfit_parameters[i][1], bestfit_parameters[i][2], bestfit_parameters[i][3], bestfit_parameters[i][4]);
        myfile.close();
    }
