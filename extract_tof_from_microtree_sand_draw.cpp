...
std::string input_name = "20220704_tofsyst_run8_kinematic_sand";
void extract_tof_from_microtree_sand_draw(){
	
	gROOT->SetBatch();

  //=== Inizialize DrawingTools                                                                         
  DrawingTools draw("/home/t2k/aspeers/PROD7_validation/testing/MultiPiAnalysis_TEST.root",4);//option 3 for T2KStyle = publication/paper
  ...
  DataSample* sand     = new DataSample("/data/aspeers/ToF_Systematics/P7_V12_FHC_run3_SAND.root");
...
TFile *ff = TFile::Open("/home/t2k/aspeers/ToF_Systematics/ToF_Output_P7_MC_run3_SAND.root","RECREATE");
...
TString Fgd1Fwdvar[] = {
  ...
  for(int ivar=0; ivar<NFgd1Fwdvar; ivar++){
    TH1D* fwd = new TH1D(Form("%s", Fgd1Fwdvar[ivar].Data()), Form("%s FWD", Fgd1Fwdvar[ivar].Data()), NBINS, TStart, Tend);
    TH1D* bwd = new TH1D(Form("%s_bwd", Fgd1Fwdvar[ivar].Data()), Form("%s BWD", Fgd1Fwdvar[ivar].Data()), NBINS, TStart, Tend); 

    draw.Draw(*sand,Form("%s[0]",Fgd1Fwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd1Fwd>0 && Fgd1Fwd_truepos[0][2] >= Fgd1Fwd_trueendpos[0][2]","","NOAUTOLABELS");
  ...
  for(int ivar=0; ivar<NFgd1Bwdvar; ivar++){
    TH1D* fwd = new TH1D(Form("%s", Fgd1Bwdvar[ivar].Data()), Form("%s FWD", Fgd1Bwdvar[ivar].Data()), NBINS, TStart, Tend);
    TH1D* bwd = new TH1D(Form("%s_bwd", Fgd1Bwdvar[ivar].Data()), Form("%s BWD", Fgd1Bwdvar[ivar].Data()), NBINS, TStart, Tend); 
    TH1D* data= new TH1D(Form("%s_data", Fgd1Bwdvar[ivar].Data()), Form("%s Data", Fgd1Bwdvar[ivar].Data()), NBINS, TStart, Tend);;
    if(ivar<2){
        draw.Draw(*sand,Form("%s[0]",Fgd1Bwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd1Bwd>0 && Fgd1Bwd_truepos[0][2] >= Fgd1Bwd_trueendpos[0][2]","","NOAUTOLABELS");
        fwd->Add(draw.GetLastStackTotal());
        draw.Draw(*sand,Form("%s[0]",Fgd1Bwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd1Bwd>0 && Fgd1Bwd_truepos[0][2] >= Fgd1Bwd_trueendpos[0][2] ","","NOAUTOLABELS");
        fwd->Add(draw.GetLastStackTotal());
        ...
        draw.Draw(*sand,Form("%s[0]",Fgd1Bwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd1Bwd>0 && Fgd1Bwd_isECAL!=-1 && Fgd1Bwd_truepos[0][2] >= Fgd1Bwd_trueendpos[0][2] ","","NOAUTOLABELS");
        fwd->Add(draw.GetLastStackTotal());
        draw.Draw(*sand,Form("%s[0]",Fgd1Bwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd1Bwd>0 && Fgd1Bwd_isECAL!=-1 && Fgd1Bwd_truepos[0][2] >= Fgd1Bwd_trueendpos[0][2] ","","NOAUTOLABELS");
        fwd->Add(draw.GetLastStackTotal());
  ...
  for(int ivar=0; ivar<NFgd1HAFwdvar; ivar++){
    TH1D* fwd = new TH1D(Form("%s", Fgd1HAFwdvar[ivar].Data()), Form("%s FWD", Fgd1HAFwdvar[ivar].Data()), NBINS, TStart, Tend);
    TH1D* bwd = new TH1D(Form("%s_bwd", Fgd1HAFwdvar[ivar].Data()), Form("%s BWD", Fgd1HAFwdvar[ivar].Data()), NBINS, TStart, Tend); 
    
    draw.Draw(*sand,Form("%s[0]",Fgd1HAFwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd1HAFwd>0 && Fgd1HAFwd_truepos[0][2] >= Fgd1HAFwd_trueendpos[0][2] && Fgd1HAFwd_EndStop_ECALSMRDFV != -1","","NOAUTOLABELS");
    fwd->Add(draw.GetLastStackTotal());
    draw.Draw(*sand,Form("%s[0]",Fgd1HAFwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd1HAFwd>0 && Fgd1HAFwd_truepos[0][2] >= Fgd1HAFwd_trueendpos[0][2] && Fgd1HAFwd_EndStop_ECALSMRDFV != -1","","NOAUTOLABELS");
    fwd->Add(draw.GetLastStackTotal());
  ...
  for(int ivar=0; ivar<NFgd1HABwdvar; ivar++){
    TH1D* fwd = new TH1D(Form("%s", Fgd1HABwdvar[ivar].Data()), Form("%s FWD", Fgd1HABwdvar[ivar].Data()), NBINS, TStart, Tend);
    TH1D* bwd = new TH1D(Form("%s_bwd", Fgd1HABwdvar[ivar].Data()), Form("%s BWD", Fgd1HABwdvar[ivar].Data()), NBINS, TStart, Tend); 
    
    draw.Draw(*sand,Form("%s[0]",Fgd1HABwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd1HABwd>0 && Fgd1HABwd_truepos[0][2] >= Fgd1HABwd_trueendpos[0][2] && Fgd1HABwd_StartStop_ECALSMRDFV != -1","","NOAUTOLABELS");
    fwd->Add(draw.GetLastStackTotal());
    draw.Draw(*sand,Form("%s[0]",Fgd1HABwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd1HABwd>0 && Fgd1HABwd_truepos[0][2] >= Fgd1HABwd_trueendpos[0][2] && Fgd1HABwd_StartStop_ECALSMRDFV != -1","","NOAUTOLABELS");
    fwd->Add(draw.GetLastStackTotal());
  ...
  for(int ivar=0; ivar<NFgd2Fwdvar; ivar++){
    TH1D* fwd = new TH1D(Form("%s", Fgd2Fwdvar[ivar].Data()), Form("%s FWD", Fgd2Fwdvar[ivar].Data()), NBINS, TStart, Tend);
    TH1D* bwd = new TH1D(Form("%s_bwd", Fgd2Fwdvar[ivar].Data()), Form("%s BWD", Fgd2Fwdvar[ivar].Data()), NBINS, TStart, Tend); 

    draw.Draw(*sand,Form("%s[0]",Fgd2Fwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd2Fwd>0 && Fgd2Fwd_truepos[0][2] >= Fgd2Fwd_trueendpos[0][2]","","NOAUTOLABELS");
    fwd->Add(draw.GetLastStackTotal());
    draw.Draw(*sand,Form("%s[0]",Fgd2Fwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd2Fwd>0 && Fgd2Fwd_truepos[0][2] >= Fgd2Fwd_trueendpos[0][2]","","NOAUTOLABELS");
    fwd->Add(draw.GetLastStackTotal());
  ...
  for(int ivar=0; ivar<NFgd2Bwdvar; ivar++){
    TH1D* fwd = new TH1D(Form("%s", Fgd2Bwdvar[ivar].Data()), Form("%s FWD", Fgd2Bwdvar[ivar].Data()), NBINS, TStart, Tend);
    TH1D* bwd = new TH1D(Form("%s_bwd", Fgd2Bwdvar[ivar].Data()), Form("%s BWD", Fgd2Bwdvar[ivar].Data()), NBINS, TStart, Tend); 
    if(ivar==0){
        draw.Draw(*sand,Form("%s[0]",Fgd2Bwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd2Bwd>0 && Fgd2Bwd_truepos[0][2] >= Fgd2Bwd_trueendpos[0][2] && Fgd2Bwd_StartStop_FGD1FV != 1","","NOAUTOLABELS");
        fwd->Add(draw.GetLastStackTotal());
        draw.Draw(*sand,Form("%s[0]",Fgd2Bwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd2Bwd>0 && Fgd2Bwd_truepos[0][2] >= Fgd2Bwd_trueendpos[0][2] && Fgd2Bwd_StartStop_FGD1FV != 1","","NOAUTOLABELS");
        fwd->Add(draw.GetLastStackTotal());
        ...
        draw.Draw(*sand,Form("%s[0]",Fgd2Bwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd2Bwd>0 && Fgd2Bwd_isECAL!=-1 && Fgd2Bwd_truepos[0][2] >= Fgd2Bwd_trueendpos[0][2] && Fgd2Bwd_StartStop_FGD1FV != 1","","NOAUTOLABELS");
        fwd->Add(draw.GetLastStackTotal());
        draw.Draw(*sand,Form("%s[0]",Fgd2Bwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd2Bwd>0 && Fgd2Bwd_isECAL!=-1 && Fgd2Bwd_truepos[0][2] >= Fgd2Bwd_trueendpos[0][2] && Fgd2Bwd_StartStop_FGD1FV != 1","","NOAUTOLABELS");
        fwd->Add(draw.GetLastStackTotal());
  ...
  for(int ivar=0; ivar<NFgd2HAFwdvar; ivar++){
    TH1D* fwd = new TH1D(Form("%s", Fgd2HAFwdvar[ivar].Data()), Form("%s FWD", Fgd2HAFwdvar[ivar].Data()), NBINS, TStart, Tend);
    TH1D* bwd = new TH1D(Form("%s_bwd", Fgd2HAFwdvar[ivar].Data()), Form("%s BWD", Fgd2HAFwdvar[ivar].Data()), NBINS, TStart, Tend); 
    
    draw.Draw(*sand,Form("%s[0]",Fgd2HAFwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd2HAFwd>0 && Fgd2HAFwd_truepos[0][2] >= Fgd2HAFwd_trueendpos[0][2] && Fgd2HAFwd_EndStop_ECALSMRDFV != -1","","NOAUTOLABELS");
    fwd->Add(draw.GetLastStackTotal());
    draw.Draw(*sand,Form("%s[0]",Fgd2HAFwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd2HAFwd>0 && Fgd2HAFwd_truepos[0][2] >= Fgd2HAFwd_trueendpos[0][2] && Fgd2HAFwd_EndStop_ECALSMRDFV != -1","","NOAUTOLABELS");
    fwd->Add(draw.GetLastStackTotal());
  ...
  for(int ivar=0; ivar<NFgd2HABwdvar; ivar++){
    TH1D* fwd = new TH1D(Form("%s", Fgd2HABwdvar[ivar].Data()), Form("%s FWD", Fgd2HABwdvar[ivar].Data()), NBINS, TStart, Tend);
    TH1D* bwd = new TH1D(Form("%s_bwd", Fgd2HABwdvar[ivar].Data()), Form("%s BWD", Fgd2HABwdvar[ivar].Data()), NBINS, TStart, Tend); 
    
    draw.Draw(*sand,Form("%s[0]",Fgd2HABwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd2HABwd>0 && Fgd2HABwd_truepos[0][2] >= Fgd2HABwd_trueendpos[0][2] && Fgd2HABwd_StartStop_ECALSMRDFV != -1","","NOAUTOLABELS");
    fwd->Add(draw.GetLastStackTotal());
    draw.Draw(*sand,Form("%s[0]",Fgd2HABwdvar[ivar].Data()),NBINS, TStart, Tend,"all","NFgd2HABwd>0 && Fgd2HABwd_truepos[0][2] >= Fgd2HABwd_trueendpos[0][2] && Fgd2HABwd_StartStop_ECALSMRDFV != -1","","NOAUTOLABELS");
    fwd->Add(draw.GetLastStackTotal());
